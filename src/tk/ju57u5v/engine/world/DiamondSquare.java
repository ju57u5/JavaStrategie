...
public class DiamondSquare {
	public static double[][] diamondSquare(int dataSize, double seed) {
		// size of grid to generate, note this must be a
		// value 2^n+1
		final int DATA_SIZE = dataSize;
		// an initial seed value for the corners of the data
		final double SEED = seed;
		...
		sideLength /= 2, h /= 2.0) {
			...
			for (int x = 0; x < DATA_SIZE - 1; x += sideLength) {
				for (int y = 0; y < DATA_SIZE - 1; y += sideLength) {
					...
					double avg = data[(x - halfSide + DATA_SIZE - 1) % (DATA_SIZE - 1)][y] + // left
																								// of
																								// center
			...
			for (int x = 0; x < DATA_SIZE - 1; x += halfSide) {
				...
				for (int y = (x + halfSide) % sideLength; y < DATA_SIZE - 1; y += sideLength) {
					...
					double avg = data[(x - halfSide + DATA_SIZE) % DATA_SIZE][y] + // left
																					// of
																					// center
